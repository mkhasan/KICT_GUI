...
//extern CString m_position;
extern SYSTEMTIME tm;
/////////////////////////////////////////////////////////////////////////////
// COScopeCtrl
...
/// ADD		
	...
	m_crGridColor = RGB(220, 220, 220);  // see also SetGridColor
...
//	m_font0.CreateFont(44, 0, 0, 0, FW_NORMAL , FALSE, FALSE, 0, DEFAULT_CHARSET,  //시간
...
void COScopeCtrl::InvalidateCtrl(bool deleteGraph)
{
	...
	m_rectPlot.left = m_rectClient.left + 8*7-15;//(nCharacters) ;//m_rectPlot.left = m_rectClient.left + 8*7+4;//(nCharacters) ;
	...
	yUnitFont.CreateFont(14, 0, 900, 900, FW_HEAVY, FALSE, FALSE, 0, DEFAULT_CHARSET,
	...
	if (m_str.YMax.IsEmpty())
		strTemp.Format(_T("%.lf"), m_PlotData[0].dUpperLimit);//strTemp.Format(_T("%.*lf"), m_nYDecimals, m_PlotData[0].dUpperLimit);
	else
		strTemp = m_str.YMax;
    ...
    if (m_rectPlot.Height() / (m_nYGrids + 1) >= 14) {
	    for (j = 1; j < (m_nYGrids + 1); j++) {
		    GridPos = m_rectPlot.Height() * j / (m_nYGrids + 1) + m_rectPlot.top;
    	    strTemp.Format(_T("%.lf"),  m_PlotData[0].dLowerLimit + (m_PlotData[0].dUpperLimit - m_PlotData[0].dLowerLimit) * (m_nYGrids - j + 1) / (m_nYGrids + 1));
    	    m_dcGrid.TextOut(m_rectPlot.left - 4, GridPos - 7, strTemp);
        }
    } else {
	    strTemp.Format(_T("%.lf"),  m_PlotData[0].dLowerLimit + (m_PlotData[0].dUpperLimit - m_PlotData[0].dLowerLimit) / 2); 
	    m_dcGrid.TextOut(m_rectPlot.left - 2, m_rectPlot.bottom + ((m_rectPlot.top - m_rectPlot.bottom) / 2) - 7 , strTemp);
    }	
	
	// y min
	if (m_str.YMin.IsEmpty())
		strTemp.Format(_T("%.lf"),  m_PlotData[0].dLowerLimit);
	else
	  strTemp = m_str.YMin;
	m_dcGrid.TextOut(m_rectPlot.left - 4, m_rectPlot.bottom - 7, strTemp);

	strTemp = m_str.XMin;  //여기
	m_dcGrid.TextOut(m_rectPlot.left - 4, m_rectPlot.bottom + 7, strTemp);
	strTemp = m_str.XMax;  //여기
	m_dcGrid.TextOut(m_rectPlot.right - 4, m_rectPlot.bottom + 7, strTemp);
	...
	m_dcGrid.TextOut((m_rectClient.left + m_rectPlot.left - 8) / 2 - rText.Height() / 2+8, //m_dcGrid.TextOut((m_rectClient.left + m_rectPlot.left - 8) / 2 - rText.Height() / 2,
	...
	int xpos = m_rectPlot.left + 10; //범례
...
//	if (deleteGraph)		//2018.1.11  m_crBackColor 지정하면 배경색이 이상하게 변하는 현상 방지.
...
void COScopeCtrl::DrawPoint()
{
	...
	CPen *oldPen;
	CFont *oldFont;
	CFont m_font0;//X-Axix position 
	CRect ScrollRect, rectCleanUp;
	m_dcPlot.SetTextColor(RGB(255,255,255));//노란색
	m_font0.CreateFont(10, 0, 900, 900, FW_NORMAL, FALSE, FALSE, 0, DEFAULT_CHARSET, //세번째 900 글자 눞힘
			OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY, DEFAULT_PITCH, _T("MS Shell Dlg"));
	oldFont = m_dcPlot.SelectObject(&m_font0);
	//static int tt=0;
...
/*		////////////////////////////////////////////////////////////////////////////////////////////////////					
		char tmp[16];
		sprintf_s(tmp, "%s", CStringA(m_time));
		if (cnt % 5 == 0 && (tmp[2]=='/')) // 데이터가 있을 때 5초마다 거리 표시
		{// 글자 사이 간격 때문에
			m_dcPlot.TextOutW(m_rectPlot.right - 10, m_rectPlot.bottom - 5, m_pos); ////m_pos 헤드파일에 추가했음
		}
		if (cnt % 60 == 0 && (tmp[2] == '/')) //1분마다 시간 표시
		{
			m_dcPlot.TextOutW(m_rectPlot.right - 10, m_rectPlot.top + 60, m_time);
		}
////////////////////////////////////////////////////////////////////////////////////////////////////*/
...
void COScopeCtrl::OnSize(UINT nType, int cx, int cy)
{
	...
	m_rectPlot.left = 20;//m_rectPlot.left = 20;???
	m_rectPlot.top = 8;
	m_rectPlot.right = m_rectClient.right - 8;
	m_rectPlot.bottom = m_rectClient.bottom - 3 - (abs(sm_logFontAxis.lfHeight) + 2)*2+5;//m_rectPlot.bottom = m_rectClient.bottom - 3 - (abs(sm_logFontAxis.lfHeight) + 2)*2 - 3;
